...
def des_encrypt():
    ...
    key = os.urandom(8)
    hkey = hexlify(key)
    print("This is your key:", hkey.decode())

    key_64bit = "".join([str(access_bit(key, i)) for i in range(len(key) * 8)])

    subkeys = generate_keys(key_64bit)

    hex_message = toHex(message)
    bin_message = hexTobinary(hex_message)

    if len(bin_message) > 64:
        print("Your message is too large, only a part of it will be encrypted")

    if len(bin_message) < 64:
        zeros = 64 - len(bin_message)
    ...
    return cipher[0:64 - zeros]


cipher = des_encrypt()
result = bin2hex(cipher)
print("Encrypted message:", result[2:])
